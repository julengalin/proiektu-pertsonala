...
public class PlayersMongoToCSV {
    public static void main(String[] args) {
        try (MongoClient mongoClient = MongoClients.create("mongodb://localhost:27017")) {
            MongoDatabase database = mongoClient.getDatabase("NBA");
            MongoCollection<Document> collection = database.getCollection("Players");
            
            Path csvFilePath = Paths.get("C:\\Users\\Julen\\OneDrive\\Escritorio\\proiektu-pertsonala\\3-Proiektu-pertsonala\\Datu-Atzipena.-2.ebaluazioa-main\\data\\csv\\players.csv");
            try (CSVWriter csvWriter = new CSVWriter(new FileWriter(csvFilePath.toFile()))) {
                ...
                if (firstDocument != null) {
                    ...
                    System.out.println("Exportación a CSV de jugadores completada con éxito.");
                } else {
                    System.out.println("La colección de jugadores está vacía. No hay datos para exportar.");
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
